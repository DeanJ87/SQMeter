...
1. Download `sqmv2-firmware-vX.Y.Z.bin` from [GitHub Releases](https://github.com/DeanJ87/SQMeter/releases)
...
!!! note "Web UI updates"
    To update the web UI (the dashboard/settings pages), flash `sqmv2-littlefs-vX.Y.Z.bin` via the web UI's **Filesystem Update** section, or use esptool directly. The web UI update doesn't touch the firmware.
...
esptool.py --chip esp32 --port PORT --baud 460800 \
  write_flash 0x10000 sqmv2-firmware-vX.Y.Z.bin

# Full reflash (nuclear option)
esptool.py --chip esp32 --port PORT --baud 460800 \
  write_flash 0x0 sqmv2-complete-flash-vX.Y.Z.bin
```
